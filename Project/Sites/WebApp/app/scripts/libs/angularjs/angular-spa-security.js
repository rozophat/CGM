angular.module('security', [ 'ngCookies'])
.constant('security.urls', {
	...
	login:                 '/Token',
	getTwoFactor:          '/api/account/getTwoFactor',
	requestCode:           '/api/account/requestCode',
...
.factory('security.api',   ['$http', 'security.urls', function ($http, Urls) {
	...
	var Api = {
		getUserInfo: function (accessToken) {
			return $http({ url: Urls.baseApi + Urls.userInfo, method: 'GET', headers: { 'Authorization': 'Bearer ' + accessToken } });
		},
		login: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.login, data: parameterize(data), headers: formdataHeader });
		},
		getTwoFactor: function () {
		    return $http({ method: 'GET', url: Urls.baseApi + Urls.getTwoFactor });
		},
		requestCode: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.requestCode, data: data });
		},
	    verifyCode: function (data) {
	        return $http({ method: 'POST', url: Urls.baseApi + Urls.verifyCode, data: data });
	    },
		logout: function () {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.logout });
		},
		register: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.join, data: data });
		},
		forgotPassword: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.forgotPassword, data: data });
		},
		resetPassword: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.resetPassword, data: data });
		},
		confirmEmail: function (data) {
		    return $http({ method: 'GET', url: Urls.baseApi + Urls.confirmEmail + '?code=' + encodeURIComponent(data.code) + '&userId=' + encodeURIComponent(data.userId) });
		},
		changePassword: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.changePassword, data: data });
		},
		getExternalLogins: function () {
		    return $http({ method: 'GET', url: Urls.baseApi + Urls.externalLogins + '?returnUrl=' + encodeURIComponent(Urls.site) + '&generateState=true', isArray: true });
		},
		manageInfo: function () {
		    return $http({ method: 'GET', url: Urls.baseApi + Urls.manageInfo + '?returnUrl=' + encodeURIComponent(Urls.site) + '&generateState=false' });
		},
		registerExternal: function (accessToken, data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.registerExternal, data: data, headers: { 'Authorization': 'Bearer ' + accessToken } });
		},
		addExternalLogin: function (accessToken, externalAccessToken) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.addExternalLogin, data: { externalAccessToken: externalAccessToken }, headers: { 'Authorization': 'Bearer ' + accessToken } });
		},
		removeLogin: function (data) {
		    return $http({ method: 'POST', url: Urls.baseApi + Urls.removeLogin, data: data });
		}
	};
...
.provider('security', ['security.urls', function (Urls) {
	...
	securityProvider.$get = ['security.api', '$q', '$http', '$location', '$timeout', '$cookieStore', function (Api, $q, $http, $location, $timeout, $cookieStore) {
		//Private Variables
	    ...
	    //Public Methods
		Security.userInfo = function() {
		    var deferred = $q.defer();

		    Api.getUserInfo().success(function (user) {

		        // handle Two Factor Authentication
		        if (user.message) {

		            //if (user.message == "TwoFactor"){
		            //    // Redirect to enter twofactor
		            //    redirectTarget(securityProvider.urls.twoFactor);
		            //} else {
		...
		Security.getTwoFactor = function() {
		    var deferred = $q.defer();

		    sessionStorage.securityProvider = null;
		    Api.getTwoFactor().success(function(providers) {
		...
		Security.requestSecurityCode = function (model) {
		    var deferred = $q.defer();

		    Api.requestCode(model).success(function (result) {

		        //Security.securityProvider = provider;
		        sessionStorage.securityProvider = model.provider;
		...
		Security.verifyCode = function (code) {
		    var deferred = $q.defer();

		    var verifyCode = { code: code, provider: sessionStorage.securityProvider };

		    Api.verifyCode(verifyCode).success(function (user) {

		        accessToken(user.access_token, false);
		        Security.user = user;
		        Security.redirectAuthenticated(redirectTarget() || securityProvider.urls.home);
		        if (securityProvider.events.login) securityProvider.events.login(Security, user); // Your Login events
		...
		Security.login = function (data) {
		    ...
		    Api.login(data).success(function (user) {
		        ...
		        if (user.twoFactor) {
		            ...
		            // Set cookies for TwoFactor
		            $cookieStore.put('.AspNet.TwoFactorCookie', user.access_token);
		...
		Security.loginWithExternal = function (login, data) {
			...
			if (securityProvider.usePopups) {
				var loginWindow = window.open(Urls.baseApi + login.url, 'frame', 'resizeable,height=510,width=380');
				...
				window.location.href = Urls.baseApi + login.url;
			}
