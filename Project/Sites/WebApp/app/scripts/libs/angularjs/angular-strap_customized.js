...
angular.module('mgcrea.ngStrap.datepicker', [
  ...
  this.$get = [
    ...
    'datepickerViews',
    '$bsTooltip',
    function ($window, $document, $rootScope, $sce, $locale, dateFilter, datepickerViews, $bsTooltip) {
      var bodyEl = angular.element($window.document.body);
      ...
      function DatepickerFactory(element, controller, config) {
        ...
        $datepicker.hide = function (blur) {
          // Phuctt Modify on 2014.09.29 start
          //$datepicker.$element.off(isTouch ? 'touchstart' : 'mousedown', $datepicker.$onMouseDown);
          //if (options.keyboard) {
          //  element.off('keydown', $datepicker.$onKeyDown);
          //}
          //_hide(blur);
          if ($datepicker.$element != undefined) {
             $datepicker.$element.off(isTouch ? 'touchstart' : 'mousedown', $datepicker.$onMouseDown);
             if (options.keyboard) {
                element.off('keydown', $datepicker.$onKeyDown);
             }
          ...
          // Phuctt Modify on 2014.09.29 end
        };
        return $datepicker;
      }
        ...
        controller.$formatters.push(function (modelValue) {
          ...
          if (angular.isUndefined(modelValue) || modelValue === null) {
            ...
            date = dateParser.parse(modelValue, null, options.modelDateFormat);
				...
				//Phat Nguyen - 31/12/2014
				//Using for month picker, if user set model from modelValue. 
				if (modelValue.length == 7) {
					var days = modelValue.split('/');
					var month = days[0];
					var year = days[1];
					modelValue = month + "/01/" + year;
				}
...
angular.module('mgcrea.ngStrap.dropdown', ['mgcrea.ngStrap.tooltip']).provider('$dropdown', function () {
  ...
  this.$get = [
    ...
    '$rootScope',
    '$bsTooltip',
    function ($window, $rootScope, $bsTooltip) {
      var bodyEl = angular.element($window.document.body);
...
// Source: modal.js
angular.module('mgcrea.ngStrap.modal', ['mgcrea.ngStrap.helpers.dimensions']).provider('$bsModal', function () {
  var defaults = this.defaults = {
      animation: 'am-fade',
  ...
  '$sce',
  '$bsModal',
  function ($window, $location, $sce, $bsModal) {
    return {
      restrict: 'EAC',
      scope: true,
      link: function postLink(scope, element, attr, transclusion) {
...
angular.module('mgcrea.ngStrap.select', [
  ...
  this.$get = [
    ...
    '$rootScope',
    '$bsTooltip',
    function ($window, $document, $rootScope, $bsTooltip) {
      var bodyEl = angular.element($window.document.body);
      var isNative = /(ip(a|o)d|iphone|android)/gi.test($window.navigator.userAgent);
      var isTouch = 'createTouch' in $window.document && isNative;
...
angular.module('mgcrea.ngStrap.timepicker', [
  ...
  this.$get = [
    ...
    'dateFilter',
    '$bsTooltip',
    function ($window, $document, $rootScope, $sce, $locale, dateFilter, $bsTooltip) {
      var bodyEl = angular.element($window.document.body);
      var isNative = /(ip(a|o)d|iphone|android)/gi.test($window.navigator.userAgent);
      var isTouch = 'createTouch' in $window.document && isNative;
      if (!defaults.lang)
...
angular.module('mgcrea.ngStrap.tooltip', ['mgcrea.ngStrap.helpers.dimensions']).provider('$bsTooltip', function () {
  ...
  '$bsTooltip',
  '$$rAF',
  function ($window, $location, $sce, $bsTooltip, $$rAF) {
    return {
      restrict: 'EAC',
      scope: true,
      link: function postLink(scope, element, attr, transclusion) {
