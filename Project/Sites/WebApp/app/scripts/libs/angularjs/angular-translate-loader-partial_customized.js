...
angular.module('pascalprecht.translate').provider('$translatePartialLoader', function () {
    ...
    Part.prototype.getTable = function (lang, $q, $http, urlTemplate, errorHandler) {
        ...
        if (!this.tables[lang]) {
            ...
            $http({
                ...
                //Phat Nguyen - 30/7/2014
                //modify for the application using purpose
                //Begin region
                var resources = data;
                data = '{';
                for (var i = 0 ; i < resources.length ; i++) {
                    data += '"' + resources[i].TextKey + '":"' + resources[i].TextValue + '",';
                };
                data += '}';
                data = data.replace(",}", "}");
	...
	//set default addPart common, 
	//From now, one screen just load only once
	parts['common'] = new Part('common'); 
