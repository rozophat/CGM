...
angular.module('pascalprecht.translate').provider('$translate', [
  ...
  function ($STORAGE_KEY) {
	  ...
	  function ($log, $injector, $rootScope, $q) {
	  	...
	  	//Phat Nguyen - 07/02/2015
	  	//Use param isInitial for checking the firstload of app, 
		//we don't need to get data if there have any addPart do after, it won't async and can't get textresource
	  	$translate.use = function (key, isFirstTranslate) {
	  		if (!key) {
	  			return $uses;
	  		}
	  		var deferred = $q.defer();
	  		...
	  		//Phat Nguyen - 26/08/2014
	  		//We are using translate partial loader, 
	  		//so if we use $translationTable[key] that not true if user uses translating at another screen
	  		//Note: still not understanding fully about this library, but my fixed working
	  		//      if any one have another solution, please ask me before editing.
	  		//if (!$translationTable[key] && $loaderFactory) {
	  		if ($loaderFactory && !isFirstTranslate) {
	  			$nextLang = key;
