...
//Modified for using enter key like tab key
//
(function (window, $) {
	'use strict';
	...
	var ngMoveSelectionHandler = function ($scope, elm, evt, grid) {
		...
		if ($scope.enableCellSelection) {
			if (charCode === 9 || charCode === 13) { //tab key
				evt.preventDefault();
			}
			...
			if (charCode === 37 || charCode === 9 && evt.shiftKey || charCode === 13 && evt.shiftKey) {
				var scrollTo = 0;
				...
				if (focusedOnFirstVisibleColumns) {
					//PhatNguyen - 12/9/2014
					//Because the our ng-grid have the orderindex and, manipulate column, so the first and the last column will not be focused
					//if (focusedOnFirstColumn && (charCode === 9 && evt.shiftKey || charCode === 13 && evt.shiftKey)) {
					if ((charCode === 9 && evt.shiftKey || charCode === 13 && evt.shiftKey)) {
			...
			else if (charCode === 39 || charCode === 9 && !evt.shiftKey || charCode === 13 && !evt.shiftKey) {
				if (focusedOnLastVisibleColumns) {
					//PhatNguyen - 12/9/2014
					//Because the our ng-grid have the orderindex and, manipulate column, so the first and the last column will not be focused
					//if (focusedOnLastColumn && (charCode === 9 && !evt.shiftKey || charCode === 13 && !evt.shiftKey)) {
					if ( (charCode === 9 && !evt.shiftKey || charCode === 13 && !evt.shiftKey)) {
		...
		if (rowIndex !== 0 && (charCode === 38 || charCode === 13 && evt.shiftKey && firstInRow || charCode === 9 && evt.shiftKey && firstInRow)) {
			offset = -1;
		}
		...
		else if (rowIndex !== items.length - 1 && (charCode === 40 || charCode === 13 && lastInRow || charCode === 9 && lastInRow)) {
			offset = 1;
		}
	...
	ngDomAccessProvider.prototype.focusCellElement = function ($scope, index) {
		if ($scope.selectionProvider.lastClickedRow) {
			...
			if (columnIndex !== undefined && elm) {
				...
				if (columns[i]) {
					//Phat Nguyen - 23/09/2014
					//To support multiple inputs in one cells, we have to style, and change some business logic of library
					//columns[i].children[1].children[0].focus();
					
					//test
					var cell = columns[i].children[1].children[0];
					//find attr "ng-class" has been defined in this library, check if true the div belongs to the library
					//otherwise is what we defined and do some business logic
					var hasNgGridAttr = cell.hasAttribute("customized-cell-template");
					if (hasNgGridAttr) {
	...
	window.ngGrid.i18n['vi'] = {
		ngAggregateLabel: 'items',
		ngGroupPanelDescription: 'Kéo tiêu đề của một cột, và thả vào đây để gom nhóm theo cột đó',
		ngSearchPlaceHolder: 'Tìm kiếm...',
		ngMenuText: 'Chọn cột:',
		ngShowingItemsLabel: 'Số container hiển thị:',
		ngTotalItemsLabel: 'Tổng số container:',
		ngSelectedItemsLabel: 'Số container đã chọn:',
		ngPageSizeLabel: 'Số trang:',
		ngPagerFirstTitle: 'Trang đầu',
		ngPagerNextTitle: 'Trang kế',
		ngPagerPrevTitle: 'Trang trước',
		ngPagerLastTitle: 'Trang cuối'
	};
	window.ngGrid.i18n['jp'] = {
		ngAggregateLabel: 'アイテム',
		ngGroupPanelDescription: '欄のヘッダを１つドラッグし、グループにするためにドロップする',
		ngSearchPlaceHolder: '検索...',
		ngMenuText: '欄を選択する:',
		ngShowingItemsLabel: 'アイテムを表示する:',
		ngTotalItemsLabel: 'アイテム数:',
		ngSelectedItemsLabel: '選択されたアイテム:',
		ngPageSizeLabel: 'ページ数:',
		ngPagerFirstTitle: '初めのページ',
		ngPagerNextTitle: '次のページ',
		ngPagerPrevTitle: '前のページ',
		ngPagerLastTitle: '最後のページ'
	};
	...
	angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
		...
		$templateCache.put('gridTemplate.html',
		  ...
		  "\n" +
		  "                    <span class=\"ngGroupName\">{{group.displayName | translate}}\r" +
		  "\n" +
		...
		$templateCache.put('headerCellTemplate.html',
		  ...
		  "\n" +
		  "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName | translate}}</div>\r" +
		  "\n" +
		...
		$templateCache.put('menuTemplate.html',
		  ...
		  "\n" +
		  "                <label><input ng-disabled=\"col.pinned\" type=\"checkbox\" class=\"ngColListCheckbox\" ng-model=\"col.visible\"/>{{col.displayName | translate}}</label>\r" +
		  "\n" +
