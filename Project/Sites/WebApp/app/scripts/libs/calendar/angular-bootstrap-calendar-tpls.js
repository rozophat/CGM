...
$templateCache.put("templates/main.html","" +
	...
	"<div class=\"alert alert-danger\" ng-if=\"!view\">The value passed to calendar-view is not set</div>" +
	"<mwl-calendar-year calendar-events=\"events\" calendar-current-day=\"currentDay\" calendar-event-click=\"eventClick\" calendar-edit-event-click=\"eventEditClick\" calendar-delete-event-click=\"eventDeleteClick\" calendar-edit-event-html=\"editEventHtml\" calendar-delete-event-html=\"deleteEventHtml\" calendar-auto-open=\"autoOpen\" ng-if=\"view == \'year\'\"></mwl-calendar-year>" +
	"<mwl-calendar-month calendar-events=\"events\" calendar-current-day=\"currentDay\" calendar-event-click=\"eventClick\" calendar-edit-event-click=\"eventEditClick\" calendar-delete-event-click=\"eventDeleteClick\" calendar-edit-event-html=\"editEventHtml\" calendar-delete-event-html=\"deleteEventHtml\" calendar-auto-open=\"autoOpen\" calendar-day-select=\"selectDay\" calendar-use-iso-week=\"useIsoWeek\" ng-if=\"view == \'month\'\"></mwl-calendar-month>" +
	"<mwl-calendar-week calendar-events=\"events\" calendar-current-day=\"currentDay\" calendar-event-click=\"eventClick\" calendar-use-iso-week=\"useIsoWeek\" ng-if=\"view == \'week\'\"></mwl-calendar-week>" +
	"<mwl-calendar-day calendar-events=\"events\" calendar-current-day=\"currentDay\" calendar-event-click=\"eventClick\" calendar-event-label=\"{{ eventLabel }}\" calendar-time-label=\"{{ timeLabel }}\" calendar-day-view-start=\"{{ dayViewStart }}\" calendar-day-view-end=\"{{ dayViewEnd }}\" ng-if=\"view == \'day\'\"></mwl-calendar-day></div>");
$templateCache.put("templates/month.html",
	"<div class=\"cal-row-fluid cal-row-head\"><div class=\"cal-cell1\" ng-repeat=\"day in weekDays\">{{ day }}</div></div>" +
	"<div class=\"cal-month-box\">" +
		"<div ng-repeat=\"week in view track by $index\">" +
			"<div class=\"cal-row-fluid cal-before-eventlist\">" +
			"<div class=\"cal-cell1 cal-cell {{ day.highlightClass }}\" ng-repeat=\"day in week track by $index\" ng-click=\"dayClicked($parent.$index, $index); selectDay()\" ng-class=\"{pointer: day.events.length > 0}\">" +
			"<div class=\"cal-month-day\" ng-class=\"{\'cal-day-outmonth\': !day.inMonth, \'cal-day-inmonth\': day.inMonth, \'cal-day-weekend\': $index == 5 || $index == 6, \'cal-day-today\': day.isToday, \'cal-day-selected\': day.isSelected}\">" +
				"<span class=\"pull-right\" " +
				"data-cal-date=\"\" " +
				//"ng-click=\"drillDown(day.label)\"" +
				">{{ day.label }}</span>" +
				
				"<small class=\"cal-events-num badge badge-info pull-left\" ng-show=\"day.warningEventNo > 0\">{{ day.warningEventNo }}</small> " +
				"<small class=\"cal-events-num badge badge-default pull-left\" ng-show=\"(day.events.length - day.warningEventNo) > 0\">{{ (day.events.length - day.warningEventNo) > 0 ?  (day.events.length - day.warningEventNo) : 0 }}</small> " +
				"<div class=\"cal-day-tick\" ng-show=\"day.isOpened\"><i class=\"glyphicon glyphicon-chevron-up\"></i> <i class=\"fa fa-chevron-up\"></i></div>" +
				//"<div class=\"events-list\" ng-show=\"day.events.length > 0\">" +
				//	"<a href=\"javascript:;\" ng-click=\"eventClick({$event: event})\" ng-repeat=\"event in day.events track by $index\" class=\"pull-left event event-{{ event.type }}\" ng-mouseenter=\"highlightEvent(event, true)\" ng-mouseleave=\"highlightEvent(event, false)\" tooltip-append-to-body=\"true\" tooltip=\"{{ event.title }}\">" +
				//	"</a>" +
				//"</div>" +
		"</div></div></div>" +
			...
			"<ul class=\"unstyled list-unstyled\">" +
				"<li ng-repeat=\"event in openEvents track by $index\">" +
					...
					"<span class=\"event-item\" " +
						...
						">" +
						"<span ng-if=\"event.title == '' || event.title == null \">" +
							"{{'LBLMAINTENANCE' | translate}}" +
						"</span>" +
						"{{ event.title }} &nbsp;" +
						"<span ng-if=\"event.objectI == undefined || event.objectI == null\">" +
							"({{'LBLDRIVER' | translate}}: {{event.objectN}})" +
						"</span>" +
						"<span ng-if=\"event.objectI == '0'\">" +
							"({{'LBLTRUCK' | translate}}: {{event.objectN}})" +
						"</span>" +
						"<span ng-if=\"event.objectI == '1'\">" +
							"({{'LBLTRAILER' | translate}}: {{event.objectN}})" +
						"</span>" +
					"</span> " +
...
angular.module('mwl.calendar')
  .service('calendarHelper', ["$filter", "moment", function calendarHelper($filter, moment) {
    ...
    this.getMonthView = function(events, currentDay, useISOWeek, selectedDate) {
      ...
      for (var i = 1; i <= numberOfDaysInMonth; i++) {
        ...
        buildRow[getWeekDayIndex(startOfMonth)] = {
			...
			isSelected: moment(selectedDate).startOf('day').isSame(startOfMonth),
			...
			warningEventNo: eventsWithIds.filter(function(event) {
          		return self.eventIsInPeriod(event.starts_at, event.ends_at, startOfMonth.clone().startOf('day'), startOfMonth.clone().endOf('day'))
          			&& event.type == "warning";
			}).length
			//infoEventNo: eventsWithIds.filter(function(event) {
        	//	return self.eventIsInPeriod(event.starts_at, event.ends_at, startOfMonth.clone().startOf('day'), startOfMonth.clone().endOf('day'))
			//		&& event.type == "info";
			//}).length
        };
  ...
  .directive('mwlCalendarMonth', ["$sce", "$timeout", "$filter", "moment", "calendarHelper", function ($sce, $timeout, $filter, moment, calendarHelper) {
    return {
      ...
      link: function postLink(scope, element, attrs, calendarCtrl) {
        ...
        function updateView() {
        	scope.view = calendarHelper.getMonthView(scope.events, scope.currentDay, scope.useIsoWeek, calendarCtrl.selectedDate);
        ...
        scope.dayClicked = function (rowIndex, cellIndex) {
        	calendarCtrl.selectedDate = scope.view[rowIndex][cellIndex].date.format("YYYY/MM/DD");
        	var isCellOpened = scope.view[rowIndex][cellIndex].isOpened;
        	var isRowOpened = scope.view[rowIndex].isOpened;
			scope.view = calendarHelper.getMonthView(scope.events, scope.currentDay, scope.useIsoWeek, calendarCtrl.selectedDate);
			scope.view[rowIndex][cellIndex].isOpened = isCellOpened;
			scope.view[rowIndex].isOpened = isRowOpened;
    ...
    return {
      ...
      controller: ["$scope", "$timeout", "$locale", "moment", function($scope, $timeout, $locale, moment) {
        ...
        $scope.control.getTitle = function() {
	      ...
	      $scope.control.getSelectedDate = function() {
